...
public class Graph_M {
	...
	public Graph_M() // he main class that represents the metro map. It contains a HashMap named
						// vtces that maps station names to their corresponding Vertex objects.
	...
	public boolean containsEdge(String vname1, String vname2) // Checks if an edge exists between two vertices.
	{
		...
		if (vtx1 == null || vtx2 == null || !vtx1.nbrs.containsKey(vname2)) {// nbrs is a data structure that stores the
																				// neighboring vertices of a particular
																				// vertex. If this condition is true, it
																				// means that the edge between vtx1 and
																				// vtx2 does not exist.
			return false;
		}
	...
	public void addEdge(String vname1, String vname2, int value) // Adds an edge between two vertices with a specified
																	// distance value.
	{
		Vertex vtx1 = vtces.get(vname1);
	...
	public boolean hasPath(String vname1, String vname2, HashMap<String, Boolean> processed) // Recursively checks if
																								// there is a path
	...
	private class DijkstraPair implements Comparable<DijkstraPair> // A helper class used for Dijkstra's algorithm,
																	// storing a vertex name, the path to that vertex,
																	// and the cost of the path
	{
		...
		// the DijkstraPair class is designed to hold information about vertices in a
		// graph along with their associated costs, and the compareTo method allows
		// instances of DijkstraPair to be compared based on their costs for use in
		// sorting or prioritizing operations.
	}
	// complete tom

	public int dijkstra(String src, String des, boolean nan) // Implements Dijkstra's algorithm to find the shortest
																// path between two stations. It returns the total cost
	...
	private class Pair // another helper class used for finding the minimum distance and time between
						// two stations.
	{
		String vname;
		String psf;
	...
	public ArrayList<String> get_Interchanges(String str)// Processes the path found by Dijkstra's algorithm to identify
															// the interchanges needed to reach the destination station.
	{
	...
	public static void Create_Metro_Map(Graph_M g) {
		...
		g.addEdge("IGI Airport~O", "New Delhi~YO", 9);
